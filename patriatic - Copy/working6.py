...
background_image = pygame.image.load(r"C:\Users\rkssp\Downloads\road (3) (1).jpg").convert()

# Load car image
car_image = pygame.image.load(r"C:\Users\rkssp\Downloads\car_image.png").convert_alpha()
...
# Initialize webcam position and dimensions
webcam_x = 20
webcam_y = 20
webcam_width = 160
webcam_height = 120

# Variable to track mouse drag state
is_dragging = False
...
def hand_gesture_control():
    global car_x, car_y, score, blue_balls, red_balls, coins, is_dragging, webcam_x, webcam_y, background_x
    ...
    while running:
        ...
        background_x -= car_speed

        # If the background goes beyond half its width, reset its position
        if background_x <= -background_image.get_width() // 2:
            background_x = 0

        # Blit webcam frame onto the screen
        display_webcam_feed()
...
# Function to capture webcam frame and display in Pygame window
def display_webcam_feed():
    global is_dragging, webcam_x, webcam_y
    ...
    if ret:
        ...
        # Resize the frame to match the dimensions of the webcam surface
        resized_frame = cv2.resize(rgb_frame, (webcam_width, webcam_height))

        # Convert the resized frame to a Pygame surface
        webcam_surface = pygame.surfarray.make_surface(resized_frame)

        # Blit the webcam surface onto the screen
        screen.blit(webcam_surface, (webcam_x, webcam_y))

        # Handle mouse events for dragging the webcam feed
        ...
        if is_dragging:
            webcam_x, webcam_y = mouse_x - webcam_width // 2, mouse_y - webcam_height // 2
        else:
            is_dragging = False
